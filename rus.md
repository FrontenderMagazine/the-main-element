...
> рассылках, твиттере, блогах, конференциях — везде.
> [Стив Фолкнер][1]
...
> [Черновик спецификации HTML для редкаторов W3C][5].
...
Так как `<main>` не предназначен для [разбиение наполнения на чати][9], он не
...
Мы рекоммендуем использоват элемент `<main>` согласно версии W3C, описанной выше.
